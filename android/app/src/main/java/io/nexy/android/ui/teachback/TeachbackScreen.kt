...
fun TeachbackScreen(conversationId: String, artifactId: String?, onBack: () -> Unit, vm: TeachbackViewModel = viewModel()) {
    ...
    Scaffold(topBar = { NexyTopAppBar(titleContent = { Text("Teach-back") }, onBack = onBack) }) { padding ->
        when (val current = state) {
            ...
            is TeachbackUiState.Practice -> PracticeContent(current, padding, record, vm::setTranscript, vm::grade)
            is TeachbackUiState.Grading -> PracticeContent(TeachbackUiState.Practice(current.exercise, current.prompt, current.transcript, current.turnNumber), padding, {}, vm::setTranscript, {}, grading = true)
            is TeachbackUiState.Feedback -> FeedbackContent(current, padding, vm::answerFollowUp, vm::tryAgain)
        }
    }
}

@Composable
private fun PracticeContent(state: TeachbackUiState.Practice, padding: PaddingValues, onRecord: () -> Unit, onTranscript: (String) -> Unit, onGrade: () -> Unit, grading: Boolean = false) {
...
@Composable
private fun FeedbackContent(state: TeachbackUiState.Feedback, padding: PaddingValues, onFollowUp: (String) -> Unit, onTryAgain: () -> Unit) {
    LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        item { TeachbackPanel {
            Text(state.prompt, style = MaterialTheme.typography.titleMedium)
