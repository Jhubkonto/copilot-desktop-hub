...
/**
 * Regression coverage for the Manual Workflow Generator's step list not being scrollable:
 ...
 * grew and squeezed the chat log toward zero height instead of scrolling. ManualWorkflowScreen.kt
 ...
 * mounting the full screen, since ManualWorkflowScreen depends on the live WsRepository singleton.
 */
@RunWith(AndroidJUnit4::class)
class ManualWorkflowStepsScrollTest {

    @get:Rule
    val composeRule = createComposeRule()

    private val steps = (1..15).map { n ->
        ManualWorkflowStepInfo(
            id = "s$n",
            title = "Step $n",
