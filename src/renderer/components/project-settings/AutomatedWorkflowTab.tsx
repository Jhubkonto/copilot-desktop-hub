...
import { Sparkles, Loader2, Braces, Check, RotateCcw, SkipForward, MessageSquare, Ban, Trash2, ArrowLeft, Clock, CheckCircle2, AlertCircle } from 'lucide-react'
import type {
  AvailableModelEntry,
  AvailableModelGroup,
...
function StepStatusBadge({ status }: { status: AutomatedWorkflowRunStep['status'] }) {
  const config = {
...
function ConfirmationModeToggle({
  mode,
  disabled,
  onChange,
}: {
...
function RunListRow({ run, onOpen, onDiscard }: { run: AutomatedWorkflowRunSummary; onOpen: () => void; onDiscard: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {projectConfig.workflowMode !== 'automated-delegation' && (
        <p className="text-[10px] text-gray-500 dark:text-gray-400">
          This project's workflow mode is set to "{projectConfig.workflowMode === 'orchestrated' ? 'Orchestrated' : 'Single'}". You can still generate a plan here — switch to Automated mode in the Team tab if you want to execute it as automated delegation.
        </p>
      )}
