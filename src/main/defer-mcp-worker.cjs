...
const port = Number(process.env.NEXY_DEFER_BRIDGE_PORT || 0)
const secret = process.env.NEXY_DEFER_BRIDGE_SECRET || ''
...
async function main() {
  ...
  const server = new Server({ name: 'nexy-defer', version: '1.0.0' }, { capabilities: { tools: {} } })
...
main().catch((error) => { process.stderr.write(`[nexy-defer] ${error.message}\n`); process.exit(1) })
