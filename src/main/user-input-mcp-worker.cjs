...
const port = Number(process.env.NEXY_UI_BRIDGE_PORT || 0)
const secret = process.env.NEXY_UI_BRIDGE_SECRET || ''
...
function post(path, body) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request({ hostname: '127.0.0.1', port, path, method: 'POST', headers: {
      ...
      res.on('end', () => { try { resolve(JSON.parse(raw)) } catch { reject(new Error('Invalid bridge response')) } })
    })
    req.on('error', reject)
    req.end(data)
  })
...
async function main() {
  ...
  const server = new Server({ name: 'nexy-user-input', version: '1.0.0' }, { capabilities: { tools: {} } })
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    try {
      ...
      return { isError: !result.success, content: [{ type: 'text', text: result.result || result.error || '(no answer)' }] }
    } catch (error) {
...
main().catch((error) => { process.stderr.write(`[user-input-mcp] ${error.message}\n`); process.exit(1) })
