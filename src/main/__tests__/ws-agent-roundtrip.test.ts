...
vi.mock('../remote-edit/history', () => ({ listHistory: vi.fn() }))
vi.mock('../remote-edit/investigator', () => ({ emitInvestigationEvent: vi.fn(), runInvestigation: vi.fn() }))
vi.mock('../remote-edit/fix-agent', () => ({ runFix: vi.fn(), emitFixEvent: vi.fn() }))
vi.mock('../remote-edit/verifier', () => ({ emitVerificationEvent: vi.fn(), runVerification: vi.fn() }))
vi.mock('../remote-edit/git-ops', () => ({ commitRemoteEditFix: vi.fn(), prepareRemoteEditCommit: vi.fn(), pushRemoteEditFix: vi.fn() }))
vi.mock('../remote-edit/recovery', () => ({ approveRelaunch: vi.fn(), getRecoveryRuns: vi.fn(), prepareReload: vi.fn(), rollbackHeal: vi.fn(), startReload: vi.fn() }))
