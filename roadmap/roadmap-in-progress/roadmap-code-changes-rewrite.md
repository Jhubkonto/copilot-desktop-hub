...
**Overall Progress**: 5 / 5 phases complete. Both platforms have a functional chat-hijack wizard as the *only* way to do a Code Change — the legacy Remote Edit screens/components/entry points have been deleted, not left running in parallel. The table/module rename remains explicitly deferred (see "Future: Table & Module Rename" below), as does a wider sweep of now-orphaned `self-heal:*`/`remote-edit:*` surface area (see Phase 5's "explicitly not done" note).
...
### PHASE 5: Cutover — Retire Legacy Remote Edit Surfaces ✅ COMPLETE

**Why**: Phases 3–4 built the new chat-hijack wizard *alongside* the old Remote Edit screens rather than replacing them. The user found this directly — a screenshot showed the old Android detail screen (phases "Draft/Planning/Patch ready/Applied/Verifying/Ready to commit/Committed", a "Planning settings" panel) still live and reachable. Investigation showed it was worse than leftover dead code: **Android's new wizard had zero entry points**, so the old screens were the *only* functioning way to create/view a code change; on desktop, both implementations ran fully in parallel (the `/code-change` slash command, a Settings → Developer "Create code change from build failure" button, and the inline `CodeChangeCard` chat bubble all still drove the old flow).

**Backend additions**:
- `startCodeChangeConversation(...)` gained an optional `initialDescription` param (threaded through `code-change:start` IPC/WS/preload/store) so entry points that already have text (slash command, build-failure button, per-message prefill) seed the wizard's describe step instead of starting blank.
- `submitDescription()` now persists the typed description to `error_reports.description` before invoking the planner — a real bug found while wiring this up: the description the user typed was never reaching the backend on **either** platform (`CodeChangeViewModel.submitDescription` on Android, and the desktop equivalent, both only ever sent `reportId`).
- Migration 74: `DELETE FROM messages WHERE role = 'system' AND content GLOB '__code-change-ref:*'` — purges old inline-card sentinel messages now that nothing renders them.

**Desktop**: `/code-change` slash command and the Settings → Developer "Create code change" button both rewritten to call `startCodeChangeConversation` (creating a dedicated conversation) instead of `captureErrorReport` + inserting a sentinel message. Deleted `CodeChangeCard.tsx`, `CodeChangeDetailView.tsx`, `CodeChangeInvestigationSection.tsx`, `RemoteEditDiffViewer.tsx`, the sentinel-detection branch in `ChatMessages.tsx`, and `deriveCodeChangePhase()`/`CodeChangeRequestPhase`/`CODE_CHANGE_PHASE_LABELS`/`CODE_CHANGE_PHASE_GUIDANCE` from `shared/code-changes.ts`. Added "Undo this change" to `CodeChangeWizard.tsx`'s plan-review/attention/final-review steps, reusing the existing `prepareRemoteEditReload`/`rollbackRemoteEdit`/`getRemoteEditRecoveryRuns` IPC channels unchanged. Added a `GitBranch` badge to code-change conversations in `ProjectHistoryPane.tsx`'s chat list.

**Android**: added `WsRepository.startCodeChange(projectId, initialDescription?)` (resolves `workspaceRoot` from the local project config, sends `code-change:start`) plus a single global `WsEvent.CodeChangeStarted`/`CodeChangeError` listener in `NavGraph.kt` that navigates to `chat/{conversationId}` or toasts an error — every entry point (the project-row icon button, the per-message "Create code change" action, the `/code-change` slash command) now just fires `startCodeChange(...)` and lets this one listener handle navigation, rather than each call site handling its own reply. Deleted `RemoteEditReportDetailScreen.kt`, `RemoteEditStartScreen.kt`, `RemoteEditReportsScreen.kt`, `RemoteEditViewModel.kt`, their tests, the `ui/remoteedit/` package, the 3 now-dead `NavGraph.kt` routes, the `ChatRenderItem.CodeChangeCard`/`CodeChangeRefBubble` inline-bubble concept (parser, render-item, bubble composable, `ChatViewModel`'s sentinel-insert logic), and the orphaned Kotlin mirror of `deriveCodeChangePhase` (`data/model/CodeChangePhase.kt` + its fixture test). Added "Undo this change" to `CodeChangeWizardHost.kt` reusing `self-heal:prepare-reload`/`self-heal:request-rollback` unchanged. Added a `CallSplit` badge to `ConversationRow` (shared by the home screen and `ScopedChatHistoryScreen.kt`).

**Cleanup sweep** (grep-verified, not blind): removed the 8 IPC channels/preload wrappers named for retirement (`error-report:find-active-for-conversation`, `remote-edit:start-fix`, `remote-edit:commit-to-workspace`, `remote-edit:revert-staged-file`, `remote-edit:mark-file-reviewed`, `remote-edit:start-verification`, `remote-edit:git-prepare-commit`, `remote-edit:git-commit`) after confirming zero remaining renderer callers for each. Their underlying engine functions (`applyStagedPatchToWorkspace`, `markStagedFileReviewed`, `runFix`, `runVerification`, `prepareRemoteEditCommit`, `commitRemoteEditFix`) were **not** deleted — they're still called either by `step-flow.ts`'s new orchestration or by Android's `self-heal:*` WS equivalents, which were deliberately left in place (see below). Rewrote/removed the desktop tests that only exercised the deleted IPC wrappers, calling the underlying functions directly instead where the function itself is still real production code.

**Explicitly not done in this pass** (scope boundary, not an oversight):
- The wider `self-heal:*` WS-command surface and the rest of the legacy `remote-edit:*` preload/IPC surface (`getStagedDiff`, `onFixEvent`/`onFixDone`, `onVerificationEvent`/`onVerificationDone`, `getRemoteEditGitStatus`, `onRemoteEditGitEvent`, and their `WsRepository.kt` Android counterparts like `getStagedDiff`, `markFileReviewed`, `remoteEditGitCommit`, `deleteRemoteEditReport`) are now **orphaned but not removed** — nothing calls them after `RemoteEditViewModel.kt`'s deletion, but a full removal touches enough surface area (backend handlers, shared types, Android `WsEvent` cases, WsEventParser cases) that it was deliberately left for a dedicated follow-up pass rather than folded into this one under time pressure. Grep for `self-heal:` in `android/app/src/main/java/io/nexy/android/data/WsRepository.kt` before starting that pass.
- The table/module rename (`error_reports` → `code_changes` etc.) remains deferred — see "Future: Table & Module Rename" below, unchanged by this phase.

**Verified**: `npm run typecheck` ✅, `npm run lint` ✅ (2 pre-existing warnings only), `npm test` ✅ (1360/1360 — count dropped from 1367 due to deleted tests for deleted code, not skipped failures), `./gradlew compileDebugKotlin` ✅, `./gradlew testDebugUnitTest` ✅.
...
- **Cutover**: ✅ Legacy Remote Edit screens/components/entry points deleted on both platforms (Phase 5); only the chat-hijack wizard remains reachable
- **Final**: `npm run typecheck` ✅, `npm run lint` ✅, `npm test` ✅ (1360/1360), `./gradlew compileDebugKotlin` ✅, `./gradlew testDebugUnitTest` ✅.
