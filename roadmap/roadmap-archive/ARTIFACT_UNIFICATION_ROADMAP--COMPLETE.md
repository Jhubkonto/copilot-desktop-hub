# Artifact Unification Roadmap --COMPLETE
...
- [x] **`src/shared/types.ts`** — Add `'artifacts'` to the `ActiveSectionPane` union type (alongside `'projects' | 'agents' | 'chats' | 'skills' | 'scheduled'`).

- [x] **`src/renderer/store/slices/uiSlice.ts`** — Remove `showArtifactsPanel: boolean` and `setShowArtifactsPanel` action. Replace all three call sites atomically in a single commit:
...
- [x] **`src/renderer/components/section-pane/ArtifactsPane.tsx`** — Create new file. Model on `SkillsPane.tsx`:
...
- [x] **`src/renderer/components/SectionPane.tsx`** — Add `'artifacts'` to `SectionType`; add `SECTION_LABELS['artifacts'] = 'Artifacts'`; import and render `<ArtifactsPane />` for the new case.

- [x] **`src/renderer/components/Sidebar.tsx`** — Change the Artifacts `NavButton` (lines ~187–195):
...
- [x] **`src/renderer/App.tsx`** — Remove lazy-imported `ArtifactsPanel` and its render call.

- [x] **`src/renderer/components/ArtifactsPanel.tsx`** — Delete file (now dead).
...
- [x] **`src/renderer/components/section-pane/ArtifactListItem.tsx`** (or inline in `ArtifactsPane.tsx`) — Create lean row:
...
- [x] **`src/renderer/store/slices/uiSlice.ts`** — Add:
  - `viewingArtifactId: string | null`
  - `openArtifactPanel(id: string) => void`
  - `closeArtifactPanel() => void`

- [x] **`src/renderer/components/ArtifactPanel.tsx`** — Create new right-side panel. Model on `AgentPanel.tsx`:
...
- [x] **`src/renderer/App.tsx`** — Lazy-import `ArtifactPanel`; render when `viewingArtifactId !== null`.

- [x] **`src/renderer/components/section-pane/ArtifactsPane.tsx`** — Wire `ArtifactListItem` rows; row click calls `openArtifactPanel`.

- [x] **Storage root config** — Move the get/set storage-root UI from `ArtifactsBrowser.tsx` into the **Details** tab footer of `ArtifactPanel` (or a global "Artifacts" section in Settings). Remove it from the list view entirely.

- [x] **`src/renderer/components/artifacts/ArtifactsBrowser.tsx`** — Deleted. Replaced `ProjectSettingsPanel.tsx` artifacts tab with new `ProjectArtifactsTab` component (`src/renderer/components/project-settings/ProjectArtifactsTab.tsx`) using the lean `ArtifactListItem` + `openArtifactPanel` pattern.
...
- [x] **`src/renderer/components/ArtifactGeneratorModal.tsx`** — Add state variables:
...
- [x] **EditForm component** (inside `ArtifactGeneratorModal.tsx`) — Fields:
  - `title` (text input)
...
- [x] **CreationOverlay component** (inside `ArtifactGeneratorModal.tsx`) — Copy from `AgentGeneratorModal.tsx` lines 112–156. Steps:
  ```
  ['Creating artifact…', 'Writing files…', 'Finalizing…', 'Done ✓']
  ```
  Show when `isGenerating && !isDone`.

- [x] **DoneOverlay component** (inside `ArtifactGeneratorModal.tsx`) — Copy from `AgentGeneratorModal.tsx` lines 342–440. Adapt:
...
- [x] **`handleGenerate` flow** — Revamp to keep modal open during generation:
...
- [x] **IPC: `artifactMoveToProject`** — Verify or add in `src/main/artifacts.ts`:
...
- [x] **`ArtifactsPane.tsx`** — Pass `projectId` when opening the generator from a project-scoped view. Refresh list on `onArtifactCreated` callback from modal.
...
- [x] **`android/app/src/main/java/io/nexy/android/ui/home/HomeScreen.kt`** — In the `TopAppBar` `actions` lambda:
  - Remove the `DropdownMenuItem` for Artifacts (currently lines ~443–447).
  - Add a dedicated `IconButton` with `Icons.Default.Inventory2` and `contentDescription = "Artifacts"`.
  - Wire `onClick = onOpenArtifacts` (this callback already exists in the function signature and is already routed in `NavGraph.kt`).
  - Skills and Scheduled remain in overflow.

- [x] **Verify `NavGraph.kt`** (lines ~400–409) — Updated `artifact-generator` route to pass `onViewArtifacts` callback (Phase 5). No new routes required for Phase 4 itself.
...
- [x] **`android/app/src/main/java/io/nexy/android/ui/artifactgenerator/ArtifactGeneratorScreen.kt`** — Updated function signature with `onViewArtifacts: () -> Unit = {}`. In `DonePhase`:
  - Added `Button(onClick = onViewArtifacts) { Text("View artifacts") }`.
  - Added project picker via `ModalBottomSheet` with `WsRepository.projects`; on select, calls `vm.moveToProject(projectId)`.

- [x] **`ArtifactGeneratorViewModel.kt`** — Added `moveToProject(projectId: String?)` method:
  - Sends `artifact:move-to-project` WsCommand.
  - Added `movedToProjectId` to `ArtifactGeneratorUiState` (updated on `ArtifactMovedToProject` event).

- [x] **`WsEvent.kt`** — Added two new sealed class entries:
  ```kotlin
  ...
  ```
  Added parsing in `WsEventParser.kt` for `artifact:moved-to-project`.
  Added WS command handler in `src/main/ws-handlers.ts` for `artifact:move-to-project`.

- [x] **`NavGraph.kt`** — Updated the `artifact-generator` composable to pass `onViewArtifacts`:
  ```kotlin
...
- [x] **Dead code removal (Desktop)**:
  - [x] Delete `src/renderer/components/ArtifactsPanel.tsx` (replaced by `ArtifactsPane` in SectionPane).
  - [x] Delete `src/renderer/components/artifacts/ArtifactsBrowser.tsx` (replaced by `ArtifactsPane` + `ArtifactPanel`; `ProjectSettingsPanel.tsx` now uses `ProjectArtifactsTab`).
  - [x] Remove `showArtifactsPanel` / `setShowArtifactsPanel` from `uiSlice.ts`.
  - [x] Confirm `ArtifactCard.tsx` is still intact (still used in chat messages — do not delete).

- [x] **IPC surface audit** — Confirm all channels are declared in `src/shared/types.ts` `IpcChannels` + `IpcReturnMap` and have corresponding `typedInvoke` wrappers in `src/preload/index.ts`:
...
- [x] **Renderer test coverage** (`src/renderer/__tests__/`):
  - [x] `ArtifactsPane.test.tsx` — renders count, search, scope picker; calls `artifactList`; opens generator modal.
  - [x] `ArtifactPanel.test.tsx` — renders tabs; "Use in Chat" calls `requestArtifactAttach`; delete calls `artifactDelete` and closes panel.
  - [x] `ArtifactGeneratorModal.test.tsx` — EditForm renders when `isEditing`; DoneOverlay renders when `isDone`; "Generate another" resets state.
  - [x] Add `window.api.artifactMoveToProject` stub to `src/test/mocks/api.ts`.

- [x] **Main-process test coverage** (`src/main/__tests__/`):
  - [x] `artifacts.test.ts` — add test for `artifact:move-to-project` handler (valid and missing artifact cases).

- [ ] **Desktop regression checklist** (manual):
...
- [ ] **Android regression checklist** (manual):
